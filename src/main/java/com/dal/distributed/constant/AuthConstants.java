package com.dal.distributed.constant;

public final class AuthConstants {

    public static final String SECURITY_QUESTION_1 = "What is the name of your favorite movie?";

    public static final String SECURITY_QUESTION_2 = "What is your favorite color?";

    public static final String SECURITY_QUESTION_3 = "What is the name of your role model?";

    public static final String USER_DETAILS_FILE_LOCATION = "usr/dpg9/authentication/User_Profile/";



    public static final String SUCCESS = "SUCCESS";

}
